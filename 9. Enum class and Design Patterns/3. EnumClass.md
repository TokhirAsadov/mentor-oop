### **1. `Enum` nima?**
`Enum` (qisqacha enumeration) — bu maxsus class turi bo‘lib, unda oldindan belgilangan, o‘zgarmas qiymatlar to‘plami saqlanadi. Masalan, hafta kunlari, oylar, holatlar (statuslar) yoki boshqa cheklangan qiymatlar to‘plami uchun ishlatiladi. `Enum` classi Java'da `java.lang.Enum` classidan meros oladi va har bir `enum` avtomatik ravishda ushbu classning vorisi hisoblanadi.
